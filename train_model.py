...
def get_compound_weights(df):
    try:
        pos_weight = df[df['class'] == 1].sum()
    except KeyError:
        pos_weight = 0
    try:
        neg_weight = df[df['class'] == 0].sum()
    except KeyError:
        neg_weight = 0
...
def test_model():
    df = load_dataset('static/files/datasets/MM Molecular Fingerprints dataset.tsv', "M")  # if fixed_sex is M or F, this will filter the dataset to include only instances from that sex.
